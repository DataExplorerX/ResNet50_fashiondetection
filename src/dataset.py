...
class FashionDataset(Dataset):
    ...
    def __getitem__(self, index):
        image = cv2.imread(f"../input/fashion_product_images/images/{self.df['id'][index]}.jpg")
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image = self.transform(image)
        cat_gender = self.df['productDisplayName'][index]
        label_gender = self.num_list_gender[cat_gender]
        cat_master = self.df['articleType'][index]
        ...
        return {
            'image': image,
            'productDisplayName': label_gender,
            'articleType': label_master,
            'subCategory': label_sub
        }       
